...
class Graph(object):
    ...
    def distance_list(self, distance):
        """
        :param distance: dictionary, key: node, value: distance of the node to source
        :return: list of tuple (distance, node)
        """
        dist_list = []
        for key in distance:
            dist_list.append((distance[key], key))
        return dist_list
