...
class multiplication(object):
    def add_index_matrix(self, l):
        for j in range(len(l)): yield (l[j], j)

    def add_index_vector(self, l):
        for j in range(len(l)): yield (j, l[j])
    ...
    def spark_multiplication(self, matrix_file, vector_file):
        ...
        """
        sc = SparkContext('local[2]', 'hw4 mult mat')
        A = sc.textFile(matrix_file).map(eval)
        A = A.map(lambda l: (l[0], list(l[1:])))
        ...
        r = r.reduceByKey(lambda n1, n2: n1 + n2)
        r = r.map(lambda l: l[1])
        return r, sc
